...
/**
 * Controlador REST que permite buscar centros educativos. Responde a peticiones
 * HTTP en la ruta /api/centros.
 */
...
@CrossOrigin // Permite que el frontend (React, Angular, etc.) haga peticiones desde otro
				// dominio o puerto
public class CentroEducativoController {
	...
	/**
	 * Constructor que recibe el servicio por inyección. Spring lo invoca
	 * automáticamente cuando crea el controlador.
	 */
	...
	/**
	 * Endpoint GET que permite buscar centros educativos. La ruta completa sería:
	 * /api/centros/buscar
	 * 
	 * Recibe parámetros opcionales: - nombre: parte del nombre del centro (texto) -
	 * provinciaId: id de la provincia - localidadId: id de la localidad
	 * 
	 ...
	 */
	@GetMapping("/buscar")
	public List<CentroEducativoDTO> buscarPorPosicion(
	        @RequestParam(required = false) Long provinciaId,
	        @RequestParam(required = false) Double latitud,
	        @RequestParam(required = false) Double longitud,
	        @RequestParam(defaultValue = "10") Double radioKm
	) {
	    return centroService.buscarCentrosConPosicion(provinciaId, latitud, longitud, radioKm);
	}
}
